  ...
  if(!window.ITEM){
    ...
    window['ITEM'] = {
      getomr:function(ito){
		...
		console.log("#################OMR1"+JSON.stringify(omr)+"  ITOKEY:"+ito.key)
        return omr;
      },
      setomr:function(ito, omr) {
        ...
        // 框架内，通知外部框架保存答案
        setTimeout(function() {
          $.publish(OBT.EVENT_ANS_CHANGED, [ito, omr]);
        }, 0);
		...
		if(done <= count){
			$("#notPaperCount").html(count - done);
			$("#completePaperCount").html(done);
		}
	  ...
	  multipleInit:function ($el,ito,settings){
        var omr = ITEM.choiceInit($el,ito,settings);
        var $inputs = $el.find('input:radio');
        //所有选择取消
        ...
        $inputs.click(ito,function(e){
		  ...
		  //OBT.storage.setLocal(ito.pcode,$(".active").attr("class"));
      ...
      /*multipleInit:function ($el,ito,settings){
        ...
        //所有选择取消
        $inputs.get(0).checked =false;
		//console.log(ITEM.getomr(ito))
		//console.log(JSON.stringify(omr))
		console.log("KEY:"+ito.key)
        ...
        $inputs.click(ito,function(e){
          ...
          if(omr.value != v){
            ...
            omr.needsave = true;
            //$omrlabels.html(vlabel==''?'&nbsp;':vlabel);
			console.log("**********KEY2:"+ito.key)
			console.log("&&&&&&&&&&&&&&ORM2:"+JSON.stringify(omr))
            ITEM.setomr(ito,omr);
			console.log("//保存客观题ITO:"+JSON.stringify(ITEM.getomr(ito)))
          }
        });
        //多项选择初始化结束
      }*/,
  ...
  var methods = {
    _init: function(options,icode) {
      ...
      if(settings.itemtype ==='single'){
        ...
        // var parentIframe = parent.document.getElementById("question_frame");
        ...
        // var parentIframe = parent.document.getElementById("question_frame");
        // parentIframe.style.height = document.body.scrollHeight + "px";
      }else if(settings.itemtype ==='smultiple'){
          //多选
          ITEM.multipleInit(this,ito,settings);
          //parent.autoHeight();
