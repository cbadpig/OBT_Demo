      ...
      addPaper:function(p,settings){
        ...
        for (var i=0;i< itemlen;i++){
          ...
          if(curans && curans.length >0){
            ...
            p.items[i].flag = ans.flag || false;
            p.items[i].needsave = ans.needsave || false;
          }else{
            //默认保存标志为true
            p.items[i].needsave = false;
            //小题标记标志
            p.items[i].flag = false;
            //保存初始化答案到Session
            var ans = '{"answer":"'+ p.items[i].answer+'","format":"'+p.items[i].format+'"}';
