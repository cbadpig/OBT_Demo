;(function () {
  ...
  if(!window.ITEM){
    ...
    window['ITEM'] = {
	  ...
	  singleClick:function($this,ito) {
		 
		  var omr = this.getomr(ito);
		  var $click = $($this);
          var v = parseInt($click.val());
          //允许取消
          if(omr.value == v){
	  ...
	  multipleClick:function($this,ito) {
          ...
          // 计算多选题答案
          $click.parent().find('input').each(function(){
            var $this2 = $(this);
            if( $this2.get(0).checked){
              v += parseInt($this2.val());
            }
          });
	  ...
	  textAreaKeyUp:function($this,itemCode) {
		var $click = $($this);
		var v = $click.val().trim();
		
		var ito;
		var papers = OBT.paper.getPapers().data;
		$.each(papers,function(k,v){
			var items = v.items;
			$.each(items,function(i,val){
				if(val.code==itemCode) {
					ito = val;
