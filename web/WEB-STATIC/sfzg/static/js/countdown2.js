    ...
    var methods = {
        ...
        getTime:function($this){
            ...
            if((timeType=='cur'||timeType=='curAll')&&(ss==-999||tt%sysnc==0)&&ss!=0){
            	...
            	console.log("ksToStart:开始");
//            	$.ajax({
//            		url:OBT.storage.getSession("basepath")+'/exam/ccKs/toStart.do',
//            		type:'post',
//            		dataType:'json',
//            		async:false,
//            		success:function(data) {
//            			if(data.status==1) {
//            				//console.log("ksToStart:"+data.message);
//            				$this.data("ss",parseInt(data.message));
//            			} else if(data.status==-41) {
//            				var url=OBT.storage.getSession("basepath")+'/exam';
//            				window.location.replace(url);
...
//            	$.ajax({
//            		url:OBT.storage.getSession("basepath")+'/exam/ccKs/kmToEnd.do',
//            		type:'post',
//            		dataType:'json',
//            		async:false,
//            		success:function(data) {
...
//            	});
        ...
        update:function($this){
        	//console.log("update:进入");
            this.getTime($this);
            //console.log("update:"+$this.data().ss);
        ...
        createHtml:function($this) {
            var data = $this.data(),timeType=data.timeType,ss=OBT.storage.getSession("ss"),s_data=data.s_data;
            if(ss<0) {
            	return ;
            }
            if(timeType=='cur') {
        ...
        format:function($this) {
            var data = $this.data(),timeType = data.timeType,ss = OBT.storage.getSession("ss");
        ...
        endTime:function($this) {
        	//console.log("endTime:进入");
            //console.log($this.data().fun);
            $this.data().fun.call();
        },
        staticTime:function(type) {
        	var sss = OBT.storage.getSession("ss");
    		if(type==1&&!sss) {
    			OBT.storage.setSession("ss",2*60);
        	} else if (type==2&&!sss) {
        		OBT.storage.setSession("ss",180*60);
				//OBT.storage.setSession("ss",16*60);
				//OBT.storage.setSession("ss",910);
        	} else if(type==-1&&sss) {
        		OBT.storage.setSession("ss",sss-1);
        	}
        }
    };
